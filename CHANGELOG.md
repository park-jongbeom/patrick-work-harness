...
> **계획을 사람이 판단할 수 있게 만든 판**이다.
> Gate A 가 내놓는 계획은 AI 가 읽기엔 충분했지만 **사람이 읽고 승인 판단을
> 내리기엔 어려웠다**. `.md` 는 정본으로 그대로 두고, 대시보드에 **읽기 전용
> 요약 패널**을 얹는다. 🔴 **승인 버튼은 넣지 않았다** — 버튼을 늘리는 설계는
> 검토가 아니라 결재 도장을 사기 때문이다(근거는 아래).

### Added

- 🔴 **Gate A 계획을 사람이 보고 판단하는 화면으로 만들었다**(`DASHBOARD-PLAN-VIZ-1`).
  **계기**: 사용자 지적 — 「계획이 .md 텍스트로만 나와서 **내가 읽고 판단하기가 어렵다**」.
  **확인**: 같은 문제가 Claude Code 이슈로 이미 등록돼 있다(`anthropics/claude-code#28288`
  — 계획은 파일에 쓰이는데 승인 프롬프트는 채팅에 떠서 *"본 적 없는 것을 승인하라고
  요구받는"* 구조. 조회 확인 2026-09-25).
  **조치**: `session-dashboard.html` 에 **「Gate A 계획 — 사람 검토용」 패널**을 넣었다.
  총계 4개(변경 파일·Step·리스크·**미검증 주장 ⓒ**)를 먼저 보여주고, 그다음 R-12 범위 가드 ·
  변경 파일 · 실행 순서 · 리스크 표 · 검증 계획을 싣는다. ⓒ가 남아 있으면 그 타일만
  주황으로 뜬다 — 사람이 가장 먼저 의심해야 할 자리다.
  **설계에서 일부러 뺀 것**: ① **승인 버튼을 넣지 않았다.** Anthropic 계측상 사용자는 권한
  프롬프트의 **약 93%를 승인**하고, *"승인을 많이 볼수록 각각에 주의를 덜 기울인다"*
  (`anthropic.com/engineering/how-we-contain-claude`, 조회 확인 2026-09-25). 버튼을 늘리면
  검토가 아니라 **결재 도장**을 사는 셈이다. 승인은 계속 대화의 문장으로 한다.
  ② **간트·타임라인을 넣지 않았다** — 코딩 에이전트 계획 검토에 쓴다는 실증 사례를 찾지 못했다.
  ③ **.md 를 대체하지 않는다.** 정본은 `CURRENT_SESSION.md` 이고 화면은 읽기 전용 요약이다
  (편집은 .md 가 제자리라는 실사용 선호가 있다).

- **Gate A 계획 파서**(`parse_gate_a_plan`) + **계획 패널 렌더러**(`render_gate_a_plan`).
  파서는 **원본 작업공간의 실제 계획 문서 909개**에서 헤딩 표기 빈도를 세어 만들었다 —
  「변경 파일」 하나만 해도 `변경 파일 목록`·`실제 변경 파일`·`변경 예상 파일`·`변경/도입 파일`
  처럼 표기가 흩어져 있어 **고정 문자열이 아니라 부분일치**로 읽는다.
  **실측 결과**: 909개 문서에서 **크래시 0건**, 변경 파일 추출 **77%**, 아무것도 못 뽑는
  문서 **16%**. 못 읽으면 패널만 빠지고 대시보드는 그대로 뜬다(무회귀).
  계획 본문은 사람이 쓴 산문이라 **HTML 이스케이프**를 새로 넣었다.

- 🔴 **계획 패널을 「잘 꾸민 목록」에서 실제 시각화로 고쳤다**(`DASHBOARD-PLAN-VIZ-2`).
  **계기**: 1차 판을 보고 사용자가 「화면은 잘 보이는데 **시각적인 자료가 없다**」고 했다.
  맞는 지적이었다 — 총계 타일 4개 말고는 전부 `● 텍스트 줄`이었고, 리스크 3건도 문장 3줄이라
  **경중 차이가 화면에 전혀 안 드러났다**. 1차는 조사의 「하지 말 것」(승인 버튼·간트)만
  지키고 **「그럼 무엇을 그릴 것인가」에는 답하지 않은** 판이었다.
  **추가한 것 3가지** — 전부 **이미 파싱해 둔 값**에서 뽑았다(새 파싱 없음):
  ① **범위 게이지**(R-12) — `2/7`·`3/5` 를 막대로. 한도 **80% 이상이면 색이 바뀐다**
  (「아직 통과」와 「곧 걸린다」는 다른 정보다. R-12 는 쪼갤지 판단하는 자리다)
  ② **실행 순서 파이프라인** — Step 을 가로 흐름(`0 → 1 → 2 → 3`)으로. 각 칸에 첫 어절을
  넣어 번호만 보고 본문을 뒤지지 않게 했다. 상세 목록은 그 아래 그대로 남는다
  ③ **파일 디렉터리 묶음 + 리스크 경중 점** — 변경이 **어디에 몰렸는지**, 어느 위험이 무거운지
  **경중 판정의 한계를 숨기지 않는다**: 판정은 본문 표현(`사소`·`낮은 위험`·`심각`)으로만 하고
  **점수를 지어내지 않는다**(일반 confidence 점수는 승인 화면 안티패턴). 실측상 계획 문서는
  낮은 쪽 표현만 쓰므로(`사소` 70 · `심각` 6 · `위험 높음` 1) **기본값은 「보통」**이고, 범례에
  **「경중 미표기 (기본)」이라고 적어 누가 정한 값인지 드러낸다** — 색만으로 뜻을 전하면
  색각 이상에서 빨강·노랑 구분이 어렵고, 말 없는 노란 점은 **AI 가 위험도를 판정한 것처럼**
  보인다.

- **시험 21건 신설**(파서 11 · 렌더러 10). 승인 버튼 부재(`<button`·`<form` 금지)와
  ⓒ 경고 타일, 미체크 범위 항목의 빨강 표시를 **계약으로 고정**했다.
  시험 총계 **161 → 182**(전건 통과).

### Fixed

- **절 구분 정규식이 바로 다음 줄의 `### 헤딩`을 삼키고 있었다.** `**굵은 라벨**` 뒤의
  공백 수량자가 `\s*` 라 줄바꿈을 넘어가, 굵은 항목 아래에 오는 절이 **통째로 사라졌다**.
  `[ \t]*` 로 좁혀 고쳤다(신설 시험이 잡아낸 결함 — 사람 눈에는 화면에서 한 절이
  조용히 없어지는 것으로만 보인다).

- 🔴 **공개 저장소에 사설 경로가 2줄 올라가 있었고, 그것이 동기화를 막고 있었다.**
  `hooks/test_install_dirs.py` 의 `_sh()` docstring 이 Git Bash 경로 함정을 설명하며
  실제 Windows 경로를 예시로 적었고, **한 줄에는 사용자명까지 들어 있었다**(커밋
  `656f8c4`, 2026-09-24 유입). **가설이 아니라 실행해서 확인했다** — `sync-from-source.sh`
  를 돌리니 `[ERROR] this repo's publishable files carry private paths` 로 두 줄을
  그대로 찍었고, 검사 범위가 변경분이 아니라 `REPO_FILES` **저장소 전체**라 유입 시점부터
  계속 걸리는 상태였다(`--apply-hooks` 거부). 드라이브 문자를 `<D>`·사용자명을 `<user>` 로
  바꿔 **교훈은 남기고 경로만 지웠다**. 수정 후 게이트 재실행 → `private paths: none.`

- 🔴 **세션 문서 3종의 차단이 이 PC 에만 있었다.** `CURRENT_SESSION.md`·`SESSION_INDEX.md`·
  `WORK_PLAN.md`·`.claude/harness-answers.yml` 이 `.git/info/exclude` 로만 막혀 있었는데,
  **`exclude` 는 커밋되지 않는다** — 새로 clone 하거나 둘째 PC 에서 작업하면 아무것도
  막지 않는다. 그 파일들에는 세션 계획과 **사설 절대경로**가 들어 있어, 그 상태에서
  `git add -A` 한 번이면 공개된다. 넷 다 `.gitignore` 로 옮겨 **규칙이 저장소를 따라다니게**
  했다. 실측 — `exclude` 를 비운 상태에서 5개 전건 `git check-ignore` 차단 확인.
  ＊`session-dashboard.html` 은 원래 `.gitignore` 에 있었다. 같은 성격의 파일 넷이 두 곳에
  갈려 있던 것이 혼란의 원인이라, 주석으로 **왜 `exclude` 가 아니라 여기인지**를 적었다.

## [1.5.2] - 2026-09-25

> **설치가 막혔을 때 사용자가 다음에 뭘 할지 알 수 있게 한 판**이다.
> 고친 것은 기능이 아니라 **안내**다 — 배포·다운로드는 원래 정상이었고,
> 막히는 것은 버전 조회 API 하나였는데 그 사실이 화면에 드러나지 않았다.

### Fixed

- 🔴 **버전 조회가 막혔을 때 설치가 아무 안내 없이 죽었다**(`HARNESS-INSTALL-RATELIMIT-1`).
  **계기**: v1.5.1 발행을 검증하다 `install.sh` 가 `[1/5]` 에서 멈추는 것을 봤다.
  **실측**: 화면에 남는 것은 `curl: (22) The requested URL returned error: 403`
  한 줄뿐이었다. `api.github.com` 의 **비인증 호출은 IP 당 시간당 60회** 제한이고,
  공용 IP·CI·잦은 재설치에서 걸린다. **친절한 `[ERROR]` 안내가 코드에 이미
  적혀 있었는데 한 번도 출력되지 않았다** — `set -euo pipefail` 아래에서
  `curl -f` 를 대입문에 직접 쓰면 403 에서 rc=22 로 끝나고, 대입 실패가 곧
  스크립트 종료라 그 아래 안내에 **도달하지 못한다**.
  **조치**: HTTP 코드를 받아 **원인을 구분**해 안내한다 — 403·429 는 한도
  초과(원인·한도·해제 시점·`--version` 우회), 000 은 연결 실패, 그 밖은 실제
  코드를 알린다. **기대 효과**: 걸려도 다음에 무엇을 할지 화면에서 알 수 있다.
  ＊배포 자체는 정상이다. 막히는 것은 **버전 조회 API** 뿐이고 tarball
  다운로드(`github.com`)는 영향이 없어, `--version <태그>` 로는 그대로 설치된다.

- **`install.sh` 버전 해석 시험 8건 신설**(`test_install_version_resolve.py`).
  `curl` 을 PATH 에 가짜로 앞세워 **200·403·429·000·404 를 결정론적으로** 잰다.
  실제 API 로는 ⓐ 레이트 리밋에 걸려 비결정적이고 ⓑ 200 이 아닌 경로를 의도적으로
  만들 수 없다(실측: 404 분기를 재려다 403 이 먼저 걸렸다). **정상 경로(200)가
  깨지지 않았는지**도 여기서 고정한다 — 안내를 넣느라 성공 경로를 망가뜨리는 것이
  더 큰 사고다.

## [1.5.1] - 2026-09-25

> v1.5.0 이 심은 집계기가 **새 프로젝트에서는 한 번도 돌지 못하던** 것을 고친 판이다.
> 세 결함 모두 **오류를 내지 않는다** — 훅은 `exit 0`, 설치는 성공 메시지를 찍는다.
> v1.5.0 의 「선언과 이행이 따로 논다」가 **한 겹 안쪽에서 반복되고 있었다**:
> 집계기는 이제 존재하는데 **쓸 자리가 없었다**.

### Fixed

- 🔴 **집계기가 새 프로젝트에서 매번 실패하고 있었다**(`HARNESS-ANALYTICS-PORT-2`).
  **계기**: v1.5.0 이 「이제 매달 쌓인다」고 적었길래 정본 경로를 열어 봤다.
  **실측**: 9월분이 **어디에도 없었다**. `write_text` 가 부모 디렉터리를 만들지 않아
  `FileNotFoundError` 로 죽는데, 호출자인 `skill-usage-auto` 훅은 예외를 stderr 로만
  남기고 **항상 exit 0** 이라 아무도 몰랐다. **Gate 를 가장 많이 쓴 세 프로젝트에
  자기 기록이 0건**이었다 — `flowlab/homepage` 는 `gate-a` 만 33회를 쓰고도 없었다.
  **조치**: 집계기에 `mkdir(parents=True)` 를 넣고(런타임 방어), **설치가 자리를
  만들도록** 했다(정본, 아래 항목). **기대 효과**: 수정 후 두 프로젝트에서 각자의
  경로에 생성됨을 실측했다.
  ＊기존 시험이 못 잡은 이유: 전부 `tempfile.mkdtemp()` 로 **이미 있는** 디렉터리에
  썼다. 실제 상황(디렉터리 없음)을 한 번도 재현하지 않았다.

- 🔴 **`install.sh` 가 훅이 쓰는 디렉터리를 만들지 않았다**(`HARNESS-INSTALL-DIRS-1`).
  **계기**: 위 결함을 각 프로젝트에서 **손으로 훅을 돌려** 메웠는데, 그건 이식이 할
  일이라는 지적을 받았다. **조치**: 설치가 `harness-answers.yml` 의
  `process_evolution_path` 를 읽어 디렉터리를 만든다. **기본값을 박지 않는다** —
  프로젝트마다 다르다(실측: `docs/process_evolution` 로 지정한 프로젝트가 있다).
  기본값을 강제하면 훅이 쓰는 곳과 **다른 자리**를 만들어, 디렉터리는 생겼는데
  집계는 계속 실패하는 상태가 된다. 상대경로는 대상 프로젝트 기준으로 해석하고,
  `--dry-run` 은 만들지 않으며, 재실행해도 기존 기록을 지우지 않는다.

- 🔴 **`_engine_version` 이 없는 답변 파일이면 설치가 조용히 죽었다.**
  **실측**: `set -euo pipefail` 아래에서 `grep` 은 매치 실패 시 rc=1 이다.
  설치가 **`[5/5]` 에서 끊긴 채 rc=1 로 종료**해 provenance 갱신·디렉터리 생성·완료
  메시지가 **전부 실행되지 않는다**. **조치**: `|| true` 로 막았다. 필드 부재는
  오류가 아니라 **처음 기록하는 경우**다.

- **provenance 를 「갱신했다」고 말해놓고 적지 않고 있었다.**
  **실측**: `sed` 는 **있는 줄만** 바꾼다. 필드가 없으면 아무 일도 안 하는데 메시지는
  「`_engine_version` 미기재 → 1.5.0」 을 출력했다. **조치**: 없으면 추가한다.

### Added

- **`run_all_tests.py` — 시험 수집 누락을 실패로 만드는 러너.**
  **실측**: `python -m unittest discover` 가 **`Ran 65 tests ... OK`** 를 출력하는데
  파일에 적힌 `def test_` 는 141개였다. 차이 76건은 `TestCase` 없이 모듈 레벨
  `def test_*()` + 자체 러너를 쓰는 **스크립트형 7파일**로, `unittest` 가 **수집
  자체를 못 한다**. 안 돌아도 실패가 아니라 **초록불**이라 줄어든 것을 알 방법이
  없었다. **조치**: 두 양식을 모두 돌리고 **정적 `def test_` 수 ≠ 실행 수면
  실패**시킨다(정적 집계는 정규식이 아니라 AST — 문자열·주석 속 `def test_` 를
  세지 않기 위해). **기대 효과**: 새 시험이 어느 양식으로 들어오든 수집에서 빠지면
  걸린다. 변이 시험 — 수동 시험 리스트에서 1건을 빼자 「정적 8 ≠ 실행 7」로 잡혔다.

- **`install.sh` 첫 시험 11건**(`test_install_dirs.py`). 이 스크립트에는 **시험이
  하나도 없었다** — 153건 중 142건이 훅 시험이었고 설치는 손으로 돌려 눈으로
  확인해왔다. v1.4.0 의 `comm` 사고(훅 설치를 건너뛰며 성공 메시지 출력)와 이번
  디렉터리 누락이 모두 그래서 출고됐다. 경로 해석 5경우·`--dry-run`·재실행 보존·
  필드 부재를 **실제 `install.sh` 실행**으로 잰다.
  ＊시험은 `--version` 을 고정해 릴리스 조회 API 를 건너뛴다 — 안 그러면 반복
  실행이 **레이트 리밋(403)** 에 걸려 코드가 멀쩡해도 무더기로 실패한다(실측).

### Changed

- **집계 윈도우를 현지 시간대로 잡는다.** 종전 `datetime.now(timezone.utc)` 는 KST
  기준 **자정~오전 9시에 날짜가 하루 밀렸다** — 9/25 01:34 에 돌린 산출물에
  「생성일 2026-09-24」 가 찍혔다(실측). **월말 자정 직후면 월 라벨까지 전달로
  밀려 엉뚱한 파일에 덮어쓴다.** v1.5.0 이 「시간대를 일반화했다」고 적었지만
  `KST` 상수는 **시간대 분포 집계에만** 쓰였고 윈도우 계산은 UTC 그대로였다 —
  일반화가 절반만 된 자리다. 비교 자체는 tz-aware 라 UTC 로도 정확하고, 틀리는
  것은 **사람이 읽는 날짜·월 라벨**이라 `HARNESS_TZ_OFFSET` 를 따르게 했다.
  ＊`--month` 경로는 여전히 UTC 로 월 경계를 잡는다. 명시 지정 경로라 지금 관측된
  결함이 아니고, 고치면 기존 5·6·7월분과 기준이 어긋나 이번에는 건드리지 않았다.
...
> v1.4.1 을 설치한 저장소의 `_engine_version` 이 `1.3.4` 로 남아, `/harness-update` 가
> **이미 설치된 것을 다시 설치하라**고 안내하는 상태였다.

### Fixed
- 🔴 **`install.sh` 가 설치 후 `_engine_version` 을 갱신한다**(`HARNESS-INSTALL-PROVENANCE-1`).
  이 스크립트는 `harness-answers.yml` 을 전혀 건드리지 않았고, 그 필드를 쓰는 것은
  `/init` 과 `/harness-update` 뿐이었다. 그래서 스크립트로 설치하면 **파일은 새 버전인데
  기록은 옛 값**으로 남았다. 이 파일은 사용자 응답이 담긴 설정이므로 **`_engine_version`
  한 줄만** 바꾸고, 수정 전 타임스탬프 백업을 남기며, 값이 이미 같으면 건너뛴다.

### Added
- **설치 표식에 버전·설치 시각을 기록한다**(`HARNESS-INSTALL-PROVENANCE-2`).
  `~/.claude/hooks/patrick-work-harness/.harness-owned` 에 `version:` · `installed_at:` 줄을
  둔다. 이전에는 문구 한 줄뿐이라 **디스크에 깔린 것이 어느 판인지 알 방법이 없었다.**
- **`/harness-update` 에 Step 1-b(교차 확인) 신설.** provenance 필드와 설치 표식을 비교해,
  어긋나면 **표식을 디스크의 사실로 보고** 필드를 맞춘 뒤 진행한다. 표식이 없으면(v1.4.2
  이전 설치) 필드만으로 판정하되 그 사실을 출력에 밝힌다.
  *필드는 「스킬이 마지막으로 주장한 것」이고 표식은 「설치기가 실제로 쓴 것」이다.*

## [1.4.1] - 2026-09-24

> v1.4.0 을 실제 대상 프로젝트에 설치하다 발견한 결함이다. **릴리스 직후 실사용에서만 드러났다** —
> `--dry-run` 은 이 경로를 타지 않는다.

### Fixed
- 🔴 **`install.sh` 가 훅을 설치하지 못하고 중단되던 문제**(`HARNESS-INSTALL-COMM-1`).
  `en_US.UTF-8` 같은 로케일에서 `sort` 는 대소문자를 무시해 `SKILL_DETAIL.md` 를 소문자 항목 뒤에
  놓는데, `comm` 은 바이트 순서(`S` < `a`)를 기대한다. 기준이 어긋나 `comm` 이
  `input is not in sorted order` 로 실패했고, `set -euo pipefail` 이라 **스크립트가 그 자리에서
  죽어 5/5 훅 설치를 통째로 건너뛰었다.** 스킬만 깔린 반쪽 상태가 「설치 완료」처럼 보였다.
  ⓐ `sort` 와 **`comm` 양쪽에** `LC_ALL=C` 를 준다(비교 기준을 실제로 쓰는 쪽이 `comm` 이라
  `sort` 에만 붙이면 안 된다 — 실측). ⓑ 이 블록은 **보고 전용**이므로 실패해도 설치를 멈추지
  않게 했다. 부가 기능이 본체를 막는 구조였다.

## [1.4.0] - 2026-09-24

> 2026-09-11 수집 근거 대조 점검에서 나온 결함을 고친 묶음이다(Phase 1). **훅 하나를 기본 구성에서
> 제거하는 파괴적 변경이 포함돼 minor 를 올린다.** 설치 경로도 플러그인 정식 경로(`hooks/hooks.json`)로
> 옮겨, 마켓플레이스로 설치한 사용자에게 훅이 0개로 들어가던 문제를 해소한다.

### Removed
- **`claude-gate-guard.py` 를 기본 구성에서 제거**(`HARNESS-CROSSCHECK-FIX-1-a-4`). 면제 목록
  (`.claude/`·`.github/`·`CURRENT_SESSION`·모든 `.md`)과 `Bash` 전용 분기 때문에 실제로 걸러내는 것이
  거의 없었고, 면제를 걷어내면 하네스가 자기 자신을 고치는 것까지 막혀 오탐 비용이 이득을 넘는다.
  2026-07-27 에 오탐 22·정탐 0 으로 가드를 축소한 선례와 같은 기준이다. **Gate 순서는 절차와 Stop
  시점 검사가 지킨다** — 대체 가드를 새로 만들지 않는다는 것이 설계 원칙이다(ARCHITECTURE §하지 않는 것).

### Added
- **`hooks/hooks.json` 신설**. 플러그인이 공식 등록 경로로 훅을 선언한다. 경로에
  `${CLAUDE_PLUGIN_ROOT}` 를 쓰므로 설치 위치가 달라져도 깨지지 않고, **사용자 전역
  `settings.json` 을 건드리지 않는다.** 이전에는 이 파일이 없어 마켓플레이스 설치 시 훅이 0개였다.
  등록 훅은 10종(PreToolUse 2 · PostToolUse 1 · Stop 7)으로 `install.sh` 가 넣던 것과 같다.
- **`sync-from-source.sh` 사설 경로 검사**. 공개 후보 파일과 스테이징 훅에서 사설 절대경로가 나오면
  종료코드 1 로 끝내고 `--apply-hooks` 를 거부한다. 패턴은 사용자명 없이 구조만 쓴다. `git grep` 을
  쓰지 않는 이유는 Git Bash 가 `/` 로 시작하는 인자를 Windows 경로로 바꿔 패턴이 조용히 0건이 되기
  때문이다. 공개 후보 목록이 비면 「깨끗함」이 아니라 오류로 멈춘다(fail-closed).
- `LICENSE` 파일(MIT).

### Changed
- **`sync-from-source.sh` 기본 실행이 보고 전용으로 바뀜**. 원본 스킬·훅을 임시 스테이징에 복사·정화한
  뒤 차이만 출력하고 저장소에는 쓰지 않는다. 스킬 자동 복사는 제거했다 — 배포본이 sed 변환을 거친
  일반화 상위형이라 통짜 복사 시 역행한다. 훅 반영은 `--apply-hooks` 에서만 하고, 원본에 없는 훅을
  지우지 않고 보고만 한다.
- **rsync 의존 제거** — 스테이징 복사에 `tar --exclude` 를 쓴다. rsync 가 없는 환경(Windows Git Bash
  기본 설치)에서 1단계 `exit 127` 로 멈추던 문제가 해소된다.
- README 스킬 수 표기 정정(11종 → 9종).

### Fixed
- 🔴 **`install.sh` 가 사용자 자산을 지우던 문제**. ⓐ 스킬 동기화의 `--delete` 를 없앴다 — 배포본에
  없는 항목을 **보고만 하고 지우지 않는다**(사용자가 직접 만든 스킬이 사라지던 결함). ⓑ 훅 전용 폴더는
  **소유 표식(`.harness-owned`)이 있을 때만** 정리한다. ⓒ 전역 `settings.json` 은 수정 전에 타임스탬프
  백업을 남긴다. 이 파일에는 하네스와 무관한 사용자 설정이 함께 살기 때문에, 병합이 잘못되면 되돌릴
  방법이 없었다.
- **`test-tampering-guard` Pattern D 오탐 완화**(`HARNESS-CROSSCHECK-FIX-1-a-3`). 패턴을 **실행 파일**
...
### Fixed
- **`skills/gate-a/SKILL.md` — 인덱스 예외 절에 완료 시 되돌리기 의무가 없던 문제 (HARNESS-AUDIT62-FIX-1)**. v1.3.3의 「Gate A 착수 시 L7·§4 갱신」 예외는 완료 시 ✅E로 되돌리는 규정이 없어, 그 규칙을 만든 세션 자신이 인덱스에 「승인 대기」로 고착되는 사고가 발생했다. 「세션이 ✅E에 도달하면 같은 Gate E 턴에서 두 값을 되돌린다」 1문장을 추가했다 — `master-plan-stale-guard.py` Stop 훅은 착수 시 미갱신만 차단하고 이 역방향 전환은 강제하지 않으므로, 문구로 명시하지 않으면 방치된 세션이 거짓 「승인 대기」 상태로 굳는다.

### Added
- **`skills/gate-a/SKILL.md` — Gate A 계획 문장의 주장-근거 대조 자기점검 `0-Claim` 신설 (HARNESS-CLAIM-EVIDENCE-1)**. 계획서의 각 사실 주장(파일/줄 수·기존 동작·영향 범위 등)을 ⓐ실측/ⓑ파일인용/ⓒ추론으로 표기하고, ⓒ만 실측으로 승격한 후 출력한다. 판정 기준은 「`파일:줄`을 제시할 수 있는가」라는 반증 가능한 형태로 고정 — 반증 불가능한 지시는 모델이 자기 추론을 ⓑ로 자칭하게 두기 때문이다. ⓒ가 0건이면 1줄로 종료(0-OSS 「N/A」 패턴과 동일하게 자연 스킵)해 비용은 추론이 실제 있을 때만 발생한다.

### Note
- 검증: 배포본 로컬 값 유입 0건·플레이스홀더/`plan_tier` 분기 보존·`master-plan-stale-guard.py` 실동작 `exit 0`(자세한 내역은 로컬 정본 archive `HARNESS-AUDIT62-FIX-1`·`HARNESS-CLAIM-EVIDENCE-1` 참조).
